...
Item {
  ...
  readonly property string agentsConfigDir: configHome + "/omarchy/agents"
  ...
  function updateCommand(kind, agentIds) {
    // Bare "omarchy-agent-usage-update" PATH-resolves to whichever copy
    // comes first in this process's environment — and Quickshell's own PATH
    // (set at session start, not this user's interactive shell rc) puts
    // /usr/share/omarchy/bin ahead of ~/.local/bin. That copy only scans
    // $OMARCHY_PATH/bin, so it silently never discovers any collector living
    // in ~/.local/bin — every provider added there was running only when
    // manually invoked outside this shell, never on the panel's own timer or
    // triggers. Going straight at the absolute path sidesteps the ambiguity
    // instead of depending on session PATH ordering this plugin doesn't
    // control.
  ...
  // ---------------------------------------------------------- credentials
  //
  // Providers with no signed-in CLI of their own (OpenRouter, OpenCode Go,
  // Ollama Cloud) take a manually-entered API key from the panel's settings
  // screen instead. The write goes through omarchy-agent-credential-set —
  // an external command owns the atomic, 0600 file write, so this stays a
  // thin Process wrapper — and a successful save immediately re-runs just
  // that one collector, so the tab reflects the new key without waiting out
  // the refresh timer.
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      root.credentialSavingId = ""
      if (exitCode === 0) root.runUpdate("force", [id])
    }

    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("agents/credentials", text.trim())
    }
  }

  function setCredential(providerId, apiKey) {
  ...
  // Belt and suspenders: the direct trigger above covers the common case
  // (save button, this process, this run), but Nous signs in from a
  // detached terminal this shell never spawned and gets no exit notice at
  // all — and the direct trigger has no way to hear about a key added
  // outside the panel either (hand-edited file, another tool). Watching the
  // credential files themselves catches every case the same way: whoever
  // wrote it, force-refresh whatever it could have changed. The one thing
  // that also legitimately rewrites nous-credentials.json is the nous
  // collector's own token refresh, which re-triggers this — harmless, since
  // a token that was just refreshed reads as not-yet-due on the next run
  // and nothing writes again.
  FileView {
    id: credentialsWatch
    path: root.agentsConfigDir + "/credentials.json"
    watchChanges: true
    printErrors: false
    onFileChanged: root.runUpdate("force", ["openrouter", "opencode-go", "ollama-cloud"])
  }

  FileView {
    id: nousCredentialsWatch
    path: root.agentsConfigDir + "/nous-credentials.json"
    watchChanges: true
    printErrors: false
    onFileChanged: root.runUpdate("force", ["nous"])
  }
